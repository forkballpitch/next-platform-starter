...
function MarkerCluster() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                console.log('학원데이터 fetch 시작');
                const res = await fetch('/api/academies');
                const json = await res.json();
                setAcademyData(json);
                console.log('학원데이터 fetch 끝', json.length, '개 학원 데이터 로드됨');
            } catch (e) {
                console.error('❌ 학원데이터 fetch 실패', e);
            }
        };
        fetchData();
    }, []);
    ...
    // ✅ 마커 최초 생성 (처음 화면 로딩시)
    useEffect(() => {
        if (!map || !window.naver || hasRun || academyData.length === 0) return;
    ...
    `
        ...
        setup();
    }, [map, navermaps, hasRun, academyData]);
